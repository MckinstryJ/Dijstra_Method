...
'''
Description: Applied Dijkstra algorithm for shortest path.
        ...
        while len(todo) > 0:
            ...
            if track_prev == False or result == []:
                ...
                while self.current != None:
                    result[len(result)-1].append(self.current.value)
                    self.current = self.current.previous
            for edge in minimum.edges:
                new_distance = minimum.distance + edge.weight
